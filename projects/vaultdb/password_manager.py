...
def create_password(pass_key, service, admin_pass):
    return sha256(admin_pass.encode('utf-8') + service.lower().encode('utf-8') + pass_key.encode('utf-8')).hexdigest()[:15] # len of pass
...
def add_password(service, admin_pass):
    ...
    command = 'INSET INTO KEYS (PASS_KEY) VALUES (%s);' %('"' + secret_key + '"')
...
if connect == ADMIN_PASSWORD:
    try:
        conn.execute(''' CREATE TABLE TABLE KEYS (PASS_KEY TEXT PRIMARY KEY NOT NULL);''')
